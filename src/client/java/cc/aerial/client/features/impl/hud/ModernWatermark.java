...
public final class ModernWatermark {
    public static final ModernWatermark INSTANCE = new ModernWatermark();
    ...
    private static final char KEYBOARD_ICON = 0xE312;
    ...
    private static void ensureAssetsLoaded() {
        if (font == null) {
            ...
            iconFont = AerialFont.createIconFromResource("OpalMaterialIconsRegular.ttf",
                    (char) 0xE7FD, (char) 0xE84E, (char) 0xE192, KEYBOARD_ICON);
            logo = AerialImage.fromResource("aerial_logo.png");
        }
    }

    public void render(Render2DEvent event) {
        AerialBloomFilter.begin(BlurConsumer.MODERN_WATERMARK);
        try {
    ...
    private void onRender2DBody(Render2DEvent event) {
        ...
        BindingService.BindKey keybind = BindRepository.INSTANCE.getBindingService().getKeyFromBindable(ClickGuiModule.INSTANCE);
        String keybindText = keybind == null ? "" : BindRepository.INSTANCE.getNameFromInteger(keybind.code(), keybind.type());
        ...
        float keybindIconW = iconFont.stringWidth(String.valueOf(KEYBOARD_ICON), ICON_FONT_SIZE);
        float keybindW = font.stringWidth(keybindText, TEXT_FONT_SIZE);
        ...
        float sec5W = keybindIconW + ICON_TEXT_GAP + keybindW;

        float totalW = PAD_X * 2
                + sec1W + ITEM_GAP + SEP_WIDTH + ITEM_GAP
                + sec2W + ITEM_GAP + SEP_WIDTH + ITEM_GAP
                + sec3W + ITEM_GAP + SEP_WIDTH + ITEM_GAP
                + sec4W + ITEM_GAP + SEP_WIDTH + ITEM_GAP
                + sec5W;

        float x = LEFT_MARGIN;
        float y = TOP_MARGIN;

        AerialBlur.drawGlass(extractor, BlurConsumer.MODERN_WATERMARK, x, y, totalW, PILL_HEIGHT,
        ...
        TextRenderUtil.drawString(extractor, iconFont, String.valueOf(KEYBOARD_ICON), curX, y + ICON_OFFSET_Y, ICON_FONT_SIZE, accentCol);
        curX += keybindIconW + ICON_TEXT_GAP;
        TextRenderUtil.drawString(extractor, font, keybindText, curX, y + TEXT_OFFSET_Y, TEXT_FONT_SIZE, textCol);
    }
}
